...
export function ItemTime({ className }: { className?: string }) {
    ...
    return (
        <Drawer open={isOpen} onOpenChange={setIsOpen}>
            <DrawerTrigger className={cn("w-full h-[128px] bg-gradient-to-b from-[#A2F21D] via-[#1B9E98]/50 to-[#162016] rounded-2xl p-[1px] cursor-pointer outline-none", className)}>
            ...
            <DrawerContent className="bg-[#161714] !rounded-t-[32px] border-t-2 border-[#2f302e] pt-3">
                ...
                <div className="relative inline-flex justify-around items-center w-full">
                    <div className="font-pixel flex flex-col gap-1 w-[98px]">
                        ...
                        <span className="text-white/40 text-xs text-center">
                            {radioValue === '1 day' ? 'DAY' : radioValue === '1 week' ? 'WEEK' : 'YEAR'}
                        </span>
                    </div>
                ...
                <DrawerFooter className="relative mt-6 mb-4">
                    ...
                    >
                        PAY <TonIcon fill="black" />{amount} GET {radioValue}
                    </TransferTonButton>
                </DrawerFooter>
            </DrawerContent>
        </Drawer>
