...
import { DotPatternInteractive, type DotPatternHandle } from "./dot-pattern-interactive";
...
export default function BombField({
    ...
    const getFreeCells = () => {
        const free: number[] = [];
        for (let i = 0; i < cellTotal; i++) if (!takenCells.current.has(i)) free.push(i);
        return free;
    };

    const pickSafeCell = (survivors: Item[]): number | null => {
    ...
    const scheduleAuto = useCallback((it: Item) => {
        ...
        const t1 = setTimeout(() => {
            setItems(cur =>
                cur.map(x => (x.id === it.id ? { ...x, isDisappearing: true, isManual: false } : x))
            );
        }, life);
    ...
    useEffect(() => {
        ...
        const spawn = () => {
            ...
            spawned++; last = Date.now();
        };

        const tick = () => {
            if (gameEndedRef.current) return;
            if (spawned === 0 && Date.now() - t0 >= FORCE) { spawn(); return; }
    ...
    const explode = useCallback((triggerId: string, x: number, y: number) => {
        ...
        setItems(cur =>
            cur.map(x => impacted.includes(x.id) ? { ...x, isDisappearing: true, isManual: false } : x)
        );
    ...
    useEffect(() => {
        const onMove = (e: TouchEvent | PointerEvent) => {
            ...
            const hit = (document.elementFromPoint(x, y) as HTMLElement)?.closest<HTMLElement>("[data-id]");
            if (!hit) return;
            ...
            if (type === "time" && hapticFeedback.impactOccurred.isAvailable()) {
                hapticFeedback.impactOccurred(/Android/i.test(navigator.userAgent) ? "heavy" : "medium");
            }
                ...
                return (
                    <div
                        ...
                        className={cn(
                            ...
                            it.isDisappearing && it.isManual && "pointer-events-none opacity-0 scale-140"
                        )}
                        onAnimationEnd={e => {
                        ...
                        {it.isDisappearing && it.isManual && (
                            <span
                                className={cn(
                                    ...
                                    it.type === "bomb" ? "text-[#ef5252]"
                                        : it.type === "time" ? "text-[#b6ff00]"
                                            : "text-white"
                                )}
                            >
                                {label}
