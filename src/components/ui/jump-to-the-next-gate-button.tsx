...
export const JumpToTheNextGateButton = () => {
  ...
  const { year, ticket, isLockedNewGate, currentLevel } = useContext(GateContext);
        ...
        <Drawer open={isOpenDrawerInfo}>
          <Container>
            ...
            >
              ...
              >
                <BuyIcon />
                <span className='ml-3'>Buy 1 {year > 0 ? 'Ticket' : 'year'} and open the gate</span>
              </ActionButton>
            </GateDrawerContent>
          </Container>
        </Drawer>
...
function GateDrawerBuyResource({ resource, imgUrl }: { resource: string; imgUrl?: string }) {
  return (
    <div>
      <div className='relative mb-8 flex justify-center'>
        {resource === 'time' ? (
          <img src={imgUrl} alt={`${resource}-image`} width={154} height={166} />
        ) : (
      ...
      <div className='font-pixel flex items-center justify-evenly font-[400] text-[white] uppercase'>
        <div>
          <p className='text-[30px] leading-[120%] text-[#B6FF00] [text-shadow:_0px_4.00224px_8.00448px_rgba(182,_255,_0,_0.3),_0px_0px_24.0134px_#B6FF00]'>
            {resource === 'time' ? <span>01</span> : <span className='mr-3'>1</span>}
          </p>
          {resource === 'ticket' ? (
            <span className='text-[16px] leading-[20px] text-[#FFFFFF]/40'>ticket</span>
          ) : (
            <span className='text-[16px] leading-[20px] text-[#FFFFFF]/40'>years</span>
          )}
        </div>
        <span className='text-[48px] text-[#FFFFFF]/40'>:</span>
        <div>
          <p className='text-[30px] leading-[120%]'>2</p>
          <span className='text-[16px] leading-[20px] text-[#FFFFFF]/40'>ton</span>
        </div>
      </div>
    </div>
  );
