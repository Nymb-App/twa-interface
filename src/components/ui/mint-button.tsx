  ...
  return (
    <button
      ...
      className={cn(
        ...
        isMinted && 'bg-gradient-to-b from-white to-[#999999] disabled:from-white disabled:to-[#999999]',
        className,
      )}
    >
      {
        isMinting ? <TextWithAppearingDots text={isTransferLoading || isTransactionLoading ? 'TRANSFER TON' : 'MINTING'} /> :
        isMinted ? 'ALREADY MINTED' : `MINT FOR ${collectionData?.mintAmount ?? 1} TON`
      }
    </button>
  )
}
