...
function toRGBA(color: string, a: number) {
    ...
    if (color.startsWith("rgb(")) return color.replace(/rgb\(([^)]+)\)/, (_, inner) => `rgba(${inner}, ${a})`);
    // hex
    let c = color.replace("#", "");
...
const FrostScratchPanel: React.FC<FrostScratchPanelProps> = ({
    ...
    const [revealedUncontrolled, setRevealedUncontrolled] = useState(defaultRevealed);
    ...
    useEffect(() => {
        setBgReady(false);
        if (!backgroundImage) { bgRef.current = null; setBgReady(true); return; }
    ...
    }, [revealed, brushRadius, brushHardness, eraseStrength, tintColor, tintOpacity, bgReady, backgroundFit, backgroundPosition]); // ★ NEW: зависимости
    ...
    const fillCover = useCallback(() => {
        ...
        if (img && img.naturalWidth && img.naturalHeight) {
            ...
            // простая позиция
            if (backgroundFit === "fill") { dx = 0; dy = 0; dw = canvas.width; dh = canvas.height; }
    ...
    const computeProgress = useCallback(() => {
        ...
        const pctx = pc.getContext("2d", { willReadFrequently: true }) as CanvasRenderingContext2D | null;
        if (!pctx) return 0;
    ...
    const notifyProgress = useCallback(
        ...
        [computeProgress, onProgress, onReveal, revealThreshold, revealedControlled, onRevealedChange]
    );
