...
function App() {
  const [isAnimationCountdownFinished, setAnimationCountdownFinished] = useState<boolean>(false);
  const [isAnimationCountdownCooldownFinished, setAnimationCountdownCooldownFinished] = useState<boolean>(false);
  ...
  return (
    <div className="w-full max-w-[450px] min-h-screen mx-auto bg-[#121312] overflow-x-hidden relative pb-6">
      ...
      >
        ...
        />
        <iframe
          className={cn('absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[70%] border-none outline-none duration-700',
    ...
    <section className='relative mt-48 text-2xl text-white px-3'>
      ...
      <div className="inline-flex justify-between gap-2 w-full mt-7 animate-slide-up-fade-1">
        {isAndroid ?
          ...
          /> :
          <GameCard delay={1000} placeholderSrc='/lottie-placeholder/minigames/slide.png' className="w-full" classNameBg='bg-[radial-gradient(ellipse_at_center,_rgba(183,_255,_0,_1)_15%,_rgba(183,_255,_0,_0.9)_30%,_rgba(183,_255,_0,_0.4)_50%,_transparent_70%)] w-[120%] h-[130%] -top-[50%] opacity-20' title="Swipes" description={'let\'s see how'} subdescription='you react' animationData={SwipeAnimationLottie} />
        }
  ...
  return (
    <section className='relative text-white px-3'>
      ...
      <div className="animate-slide-up-fade-4">
        <h3 className='text-white/60 text-sm text-center mt-2'>Get the best deals and benefits</h3>
      ...
      <div className='inline-flex w-full gap-2 mt-6 animate-slide-up-fade-5'>
        <Card className='w-full flex flex-col bg-[#161715]'>
          ...
          <span className="mt-3 text-white/60 mx-auto z-10">One for the wallet</span>
        </Card>
      </div>
    </section>
  );
  ...
  useEffect(() => {
    const handleVis = () => {
      ...
      if (document.visibilityState === 'hidden') {
        ...
        video.play().catch(() => {/* игнорим блок автоплея */ });
      }
    };
