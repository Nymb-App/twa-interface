...
import FrensImage from '/frens-img.png';
...
function RouteComponent() {
  ...
  return (
    <PageLayout>
      <header className="font-pixel relative w-full bg-[url('/frens-bg.png')] bg-bottom bg-no-repeat px-3 pb-6 font-[400]">
        <img
          src={FrensImage}
          alt='preview-image'
          width={191}
        ...
        {finishAt > 0 && !isClaimStart && (
          <Countdown
            ...
            intervalDelay={10}
            precision={3}
            renderer={(props: any) => <CountdownTimerDisplay isCountdownHeaderView {...props} />}
          />
        )}
        {!loading && !finishAt && !isClaimStart && (
          <CountdownTimerDisplay isCountdownHeaderView days={0} hours={0} minutes={0} seconds={0} />
        )}
        ...
        {isClaimStart && !isClaimEnd && (
          <TimeCountup
            ...
            targetTimestamp={Number(localStorage.getItem(NYMB_FARMING_FINISHAT_LS_KEY))}
    ...
    <>
      <div className='font-pixel mb-3 px-3 pt-[40px] pb-4'>
        <div className='flex items-center justify-between gap-2 font-[400]'>
          <h2 className='font-pixel text-[18px] leading-6 uppercase'>Referrals code</h2>
          <Button
            className={cn(
          ...
          >
            ...
            >
              <path
                ...
                fill={myCodes && myCodes.length >= 5 ? 'rgba(255,255,255,0.6)' : '#121312'}
              />
            </svg>
            add new
          </Button>
              ...
              <div className='flex items-center justify-between font-[400]'>
                <div className='font-inter'>
                  <div className='flex items-center'>
                    ...
                    <CopyButton content={`${TELEGRAM_APP_URL}?startapp=${user?.id}_${item.code}`} />
                  </div>
...
const RefferalsMembersList = ({ meTime }: { meTime: number }) => {
  ...
  return (
    <>
      <div className='font-pixel mb-3 px-3 pt-[40px] pb-4'>
        <div className='flex items-center justify-between gap-2 font-[400]'>
          <h2 className='font-pixel text-[18px] leading-6 uppercase'>{referralsCount} frens</h2>
          <Link to='/send-gift' disabled={MIN_ALLOWED_TIME >= meTime || referralsCount >= 10}>
            <Button
            ...
            >
              ...
              <span className='text-[12px] leading-4 text-[#FFFFFF] uppercase'>send gift</span>
            </Button>
          </Link>
        </div>
      </div>
