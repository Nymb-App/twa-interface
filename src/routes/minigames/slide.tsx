...
export function RouteComponent() {
  ...
  return (
    <PageLayout
      useFooter={false}
      className='h-[calc(100vh-7.5rem)] flex flex-col overflow-hidden'
    >
      {/* ───────── header ───────── */}
...
function GameFinished({
  ...
  return (
    <div className='absolute inset-0 flex flex-col items-center justify-between font-pixel bg-black/30 z-50'>
      <header className='relative w-full h-[310px]'>
        <img
          src={'/minigames/winning-stars.png'}
          className='w-full h-auto object-cover bg-blend-lighten absolute'
        />
      ...
      <div className='flex flex-col items-center justify-center gap-2 w-full px-4 pb-10'>
        <ActionButton onClick={onShare} className='text-black bg-gradient-to-b from-white to-[#999999] active:from-[#999999] active:to-[#535353] disabled:from-[#999999] disabled:to-[#535353] disabled:cursor-not-allowed'>
          SHARE AND GET +20%
        </ActionButton>

        {useRestart && 
          <ActionButton onClick={onRestart} className='text-black active:from-[#73a531] active:to-[#689100] disabled:from-[#73a531] disabled:to-[#689100] disabled:cursor-not-allowed'>
            PLAY MORE
          </ActionButton>
        }
      </div>
