...
export const useStatistics = (params?: GlobalStatisticsParams) => {
    const { get } = useApi();
    const { lvls } = useLvl();
    ...
    const levelQueries = useQueries({
        queries: lvls.map((lvl) => ({
            ...
            queryFn: async () => {
                const response = await get(`/statistics/get_global_statistics?lvl=${lvl}`);
                if (!response) {
    ...
    const globalStatistics = {
        data: levelQueries.reduce((acc, query) => {
            if (query.data) {
