...
export const useMint = () => {
  ...
  const { data: collectionData, isLoading: isCollectionDataLoading } = useQuery(
    {
      ...
      queryFn: async () => {
        try {
          const response = await get(`/nft/get-collection-data`)
  ...
  const { data: mintProgress, isLoading: isMintProgressLoading } = useQuery({
    queryKey: ['isMinted'],
    queryFn: async () => {
      try {
        const response = await get(`/nft/get-mint-progress`)
